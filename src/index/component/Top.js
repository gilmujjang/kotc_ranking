        ...
        docRef.get().then((doc) => {
          if (!doc.exists) {
            docRef
              ...
              .catch((error) => {
                alert(
                  "whole_users에 문서를 추가하는데에 실패하였습니다. : ",
                  error
                );
              });
          }
      ...
      <div className={styles.header}>
        <div
          className={classNames({
            ["container"]: true,
            [styles.container__index__header]: true,
          })}
        >
          ...
          {isSignedIn ? (
            <ul>
              <li>
                <img src={userObj.photoURL} alt="user profile" />
              </li>
              ...
              <li>
                <Icon
                  name="setting"
                  size="large"
                  className={styles.icon__setting}
                  onClick={() => {
                    setIsModalOpen(true);
                  }}
                />
              </li>
              <li className="button__index" onClick={onGoogleSignOut}>
                Sign out
              </li>
            </ul>
          ) : (
            <ul>
              <li className="button__index" onClick={onGoogleSignIn}>
                Sign in with Google
              </li>
            </ul>
          )}
