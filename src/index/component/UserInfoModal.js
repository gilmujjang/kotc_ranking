...
const UserInfoModal = ({ isModalOpen, setIsModalOpen }) => {
  ...
  function changePhotoURL(e) {
    ...
    const storageRef = storageService
      .ref()
      .child(`user profile img/${imageRef.name}`);
            ...
            storageRef
              ...
              .then((url) => {
                docRef
                  ...
                  .catch((error) => {
                    alert("이미지 변경에 실패하였습니다. : ", error);
                  });
              })
              .catch((error) => {
                alert("storage에서 이미지 url을 가져오는데 실패하였습니다. : "),
                  error;
              });
          })
          .catch((error) => {
            alert("storage에 이미지를 업로드 하는데 실패하였습니다. : "), error;
          });
      }
    }
  }
