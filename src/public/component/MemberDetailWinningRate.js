  ...
  useEffect(() => {
    ...
    if (getWinningRate() === 0) {
      ...
      drawGreenCircle(
        ...
        46,
        degToRad(270),
        degToRad(270 - 360 * getWinningRate()),
        true
      );
      drawRedCircle(
        ctx,
  ...
  return (
    <>
      ...
      <div className={styles.gameRecord}>
        <span className={styles.record__left}>
          {playerDetailTarget.game_win} 승
        </span>
        <span className={styles.record__right}>
          {playerDetailTarget.game_lose} 패
        </span>
      </div>
    </>
  );
