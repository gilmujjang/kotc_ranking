...
const MemberList = ({ groupName, groupPlayers }) => {
  const groupPlayerList = groupPlayers.concat().sort(function (a, b) {
    a.name - b.name;
  });
  const [mapList, setMapList] = useState([]);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [filterName, setFilterName] = useState("");
  const [playerDetailTarget, setPlayerDetailTarget] = useState({});
  ...
  function searchPlayerFromGroupPlayers(target) {
    const searchedTarget = groupPlayers.find(
      (player) => player.name === target.dataset.name
    );
    ...
    <>
      ...
      <div className={styles.allMembers}>
        {mapList.length === 0 ? (
          ...
          <div className={styles.memberList}>{memberListCard}</div>
        )}
        <MemberDetailModal
          ...
          setPlayerDetailTarget={setPlayerDetailTarget}
