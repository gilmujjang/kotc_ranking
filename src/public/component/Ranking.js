...
const RankingContent = ({ groupPlayers }) => {
  ...
  useEffect(() => {
    switch (currentType) {
      case "전체":
        if (filterName) {
          setPlayersToShow(
            sortedPlayers.filter((el) => el.name.includes(filterName))
          );
        } else {
  ...
  const noRankings = (
    <div className={styles.no_rankings}>
      생성된 플레이어가 한 명도 없습니다.
    </div>
  );
  ...
  const rankingFilter = (
    <div className={styles.ranking_filter_container}>
      ...
      <input
        type="text"
        className="filter__rankingContainer"
        placeholder="text me..."
        onChange={traceInput}
      />
    </div>
  ...
  return (
    <>
      ...
      {playersToShow.length === 0 ? (
        ...
        <div className={styles.rankingContainer}>{playerRankingCard}</div>
      )}
    </>
  );
};
